...
import (
	...
	"github.com/suse/elemental-lifecycle-manager/test/utils"
)

// namespace where the project is deployed in
const namespace = "elemental-lifecycle-manager-system"
...
var _ = Describe("Manager", Ordered, func() {
	...
	Context("Manager", func() {
		...
		It("should ensure the webhook endpoint is ready", func() {
			...
			verifyWebhookEndpointsReady := func(g Gomega) {
				cmd := exec.Command("kubectl", "get", "endpointslices.discovery.k8s.io", "-n", namespace,
					"-l", "kubernetes.io/service-name=elemental-lifecycle-manager-webhook-service",
			...
			verifyValidatingWebhookReady := func(g Gomega) {
				cmd := exec.Command("kubectl", "get", "validatingwebhookconfigurations.admissionregistration.k8s.io",
					"elemental-lifecycle-manager-validating-webhook-configuration",
		...
		// Consider applying sample/CR(s) and check their status.
	})
})
